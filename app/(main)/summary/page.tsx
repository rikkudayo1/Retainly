...
const SummaryPage = () => {
  ...
  React.useEffect(() => {
    ...
    if (showFileMenu) document.addEventListener("click", handleClickOutside);
  ...
  const handleSend = async () => {
    ...
    const userMessage = selectedFile
      ? `${t("summary.title")}: ${selectedFile.name}`
      : input;
    ...
    try {
      ...
      formData.append("text", selectedFile ? selectedFile.text : input);
      ...
      if (data.success) {
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: data.output.summary },
        ]);
      } else {
        setMessages((prev) => [
          ...prev,
      ...
      <div className="flex-1 overflow-y-auto px-6 py-10 space-y-6">
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-black mb-2">{t("summary.title")}</h1>
        ...
        {messages.map((msg, i) => (
          ...
          >
            ...
            <div
              className={`max-w-2xl px-4 py-3 rounded-2xl text-sm leading-relaxed ${msg.role === "user" ? "rounded-br-sm" : "rounded-bl-sm"}`}
              style={
                msg.role === "user"
                  ? {
