...
import { CheckCircle, XCircle, Trophy, Brain, Zap, FileText, ChevronDown } from "lucide-react";
import { useGems } from "@/hooks/useGems";
import { useLanguage } from "@/context/LanguageContext";
import { getFiles, DBFile } from "@/lib/db";

interface QuizQuestion { question: string; choices: string[]; answer: string; explanation?: string }
interface QuizResult { questionIndex: number; selected: string; correct: boolean }
...
const QuizPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success) {
        setQuestions(data.output.quiz); setCurrentIndex(0); setResults([]);
        setSelectedAnswer(null); setAnswered(false); setCorrect(0); setWrong(0); setGemsEarned(0);
        setPageState("quiz");
      } else { setError(data.error || t("quiz.error.failed")); }
    } catch { setError(t("quiz.error.generic")); }
  ...
  const handleAnswer = (choice: string) => {
    ...
    setSelectedAnswer(choice); setAnswered(true);
    if (isCorrect) setCorrect((p) => p + 1); else setWrong((p) => p + 1);
  ...
  const handleNext = () => {
    if (currentIndex + 1 >= questions.length) {
      ...
      addGems(earned); setGemsEarned(earned); setPageState("results");
    } else {
      setCurrentIndex((p) => p + 1); setSelectedAnswer(null); setAnswered(false);
    }
  };
  ...
  const gradientBtn: React.CSSProperties = { background: "var(--theme-gradient)", color: "#fff", textShadow: "0 1px 3px rgba(0,0,0,0.4)" };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen px-6 bg-background text-foreground">
      <div className="w-full max-w-xl space-y-6">
        <div className="text-center space-y-3">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto" style={{ background: `rgb(var(--theme-glow) / 0.1)` }}>
            <Zap className="w-7 h-7" style={{ color: "var(--theme-primary)" }} />
          </div>
          <h1 className="text-3xl font-black">{t("quiz.title")}</h1>
          <p className="text-muted-foreground text-sm">{t("quiz.subtitle")}</p>
          <p className="text-xs" style={{ color: "var(--theme-badge-text)" }}>{t("quiz.gems_hint")}</p>
        </div>

        {/* File dropdown — shows skeleton while loading */}
        {loadingFiles ? (
          <div className="h-12 rounded-xl animate-pulse" style={{ backgroundColor: `rgb(var(--theme-glow) / 0.06)` }} />
        ) : storedFiles.length > 0 && (
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-widest text-muted-foreground/60">{t("quiz.library")}</label>
            <div className="relative mt-2" onClick={(e) => e.stopPropagation()}>
              <button onClick={() => setFileDropdownOpen((p) => !p)}
        ...
        <div className="space-y-2">
          <label className="text-xs uppercase tracking-widest text-muted-foreground/60">{t("quiz.paste")}</label>
          <textarea className="mt-2 w-full rounded-xl px-4 py-3 text-sm outline-none resize-none min-h-[140px] border"
            style={{ backgroundColor: `rgb(var(--theme-glow) / 0.04)`, borderColor: `rgb(var(--theme-glow) / 0.2)` }}
            placeholder={t("quiz.placeholder")} value={pastedText}
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-6 bg-background text-foreground">
        <div className="w-full max-w-xl space-y-5">
          ...
          <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: `rgb(var(--theme-glow) / 0.1)` }}>
            <div className="h-full rounded-full transition-all duration-500" style={{ width: `${progress}%`, background: "var(--theme-gradient)" }} />
          </div>

          <div className="rounded-2xl p-6 border" style={{ borderColor: `rgb(var(--theme-glow) / 0.2)`, backgroundColor: `rgb(var(--theme-glow) / 0.05)` }}>
          ...
          <div className="space-y-2.5">
            {currentQ.choices.map((choice, i) => (
              <button key={i} onClick={() => handleAnswer(choice)}
                ...
                onMouseEnter={(e) => { if (!answered) { (e.currentTarget as HTMLButtonElement).style.borderColor = "var(--theme-primary)"; (e.currentTarget as HTMLButtonElement).style.backgroundColor = `rgb(var(--theme-glow) / 0.08)`; } }}
                onMouseLeave={(e) => { if (!answered) { (e.currentTarget as HTMLButtonElement).style.borderColor = `rgb(var(--theme-glow) / 0.2)`; (e.currentTarget as HTMLButtonElement).style.backgroundColor = `rgb(var(--theme-glow) / 0.03)`; } }}>
                {choice}
              </button>
            ))}
          </div>
            ...
            <div className="space-y-3">
              {currentQ.explanation && (
                <div className="rounded-xl px-4 py-3 text-sm border"
                  ...
                  <span className="font-semibold" style={{ color: "var(--theme-badge-text)" }}>{t("quiz.explanation")} </span>
                  <span className="text-muted-foreground">{currentQ.explanation}</span>
                </div>
              )}
              <button onClick={handleNext} className="w-full py-3 rounded-xl font-semibold text-sm hover:brightness-110 transition-all" style={gradientBtn}>
    ...
    return (
      <div className="px-6 py-16 max-w-2xl mx-auto space-y-8 bg-background text-foreground min-h-screen">
        <div className="text-center rounded-3xl p-10 border space-y-3"
          ...
          <h1 className="text-6xl font-black bg-clip-text text-transparent" style={{ backgroundImage: "var(--theme-gradient)" }}>{scorePercent}%</h1>
          <p className="text-muted-foreground text-sm">
            {correct} {t("quiz.correct")} · {wrong} {t("quiz.wrong")} {t("quiz.out_of")} {total} {t("quiz.questions")}
          </p>
          {scorePercent === 100 && (
            <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: "var(--theme-badge-text)" }}>{t("quiz.perfect")}</p>
          )}
          {gemsEarned > 0 && (
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold mt-2"
              style={{ backgroundColor: `rgb(var(--theme-glow) / 0.15)`, color: "var(--theme-badge-text)", border: `1px solid rgb(var(--theme-glow) / 0.3)` }}>
              💎 +{gemsEarned} {t("quiz.gems_earned")}
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl p-5 border space-y-3"
          ...
          <div className="flex items-center gap-2 font-semibold text-sm">
            ...
            <span style={{ color: "var(--theme-badge-text)" }}>{t("quiz.work_on")}</span>
          </div>
          <p className="text-sm text-muted-foreground leading-relaxed">{weakness}</p>
        </div>

        <div className="space-y-5">
          <h2 className="text-lg font-bold">{t("quiz.breakdown")}</h2>
          {questions.map((q, qi) => {
