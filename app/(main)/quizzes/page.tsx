...
import { Terminal, Zap, Trash2, Play, Globe, Pencil, Star, ChevronDown, Calendar } from "lucide-react";
import { getMyQuizCollection, deleteQuizAttempt, Quiz } from "@/lib/db";
    ...
    >
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <div>
          ...
          {quiz.username && (
            <p className="font-mono text-[10px] mt-1" style={{ color: `rgb(var(--theme-glow) / 0.4)` }}>
              by @{quiz.username}
            </p>
          )}
          <div
          ...
          >
            ...
            {quiz.star_count > 0 && (
              <span className="flex items-center gap-1">
                <Star className="w-3 h-3" /> {quiz.star_count}
              </span>
            )}
          </div>
        </div>
        ...
        {/* Expand toggle */}
        {(quiz.description || quiz.questions?.length > 0) && (
          <button
        ...
        {expanded && (
          ...
          >
            ...
            <div className="flex" style={{ borderColor: `rgb(var(--theme-glow) / 0.08)` }}>
                  ...
                  {quiz.questions.slice(0, 3).map((q: any, i: number) => (
                    <div key={i} className="flex items-start gap-2">
                      ...
                      <p
                        ...
                        style={{ color: `rgb(var(--theme-glow) / 0.6)`, fontSize: "11px", fontFamily: "inherit" }}
                      >
                        {q.question}
                      </p>
                    </div>
            ...
            {!loading && (
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-foreground">
                    {quizzes.filter((q) => q.user_score !== null && q.user_score !== undefined && q.user_score > 0).length}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="page-enter stagger-1">
            <SectionRule
              label={
                loading
                  ? "// LOADING..."
                  : `// ${quizzes.length} QUIZ${quizzes.length !== 1 ? "ZES" : ""} IN COLLECTION`
              }
            />
