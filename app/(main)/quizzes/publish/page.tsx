...
import { Globe, ChevronDown, Zap, Upload, Plus, Menu } from "lucide-react";
import {
  getQuizSessions,
  getPublishedQuizzes,
  publishQuiz,
...
// ── Published quiz card (own) ──────────────────────────────────
const MyPublishedQuizCard = ({
  quiz,
  onUnpublish,
}: {
  quiz: PublishedQuiz;
        ...
        >
          {confirming ? (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold font-mono transition-all hover:brightness-110"
              style={{
...
const QuizPublishPage = () => {
  const [myQuizzes, setMyQuizzes] = useState<DBQuizSession[]>([]);
  const [publishedQuizzes, setPublishedQuizzes] = useState<PublishedQuiz[]>([]);
  const [selectedQuiz, setSelectedQuiz] = useState<DBQuizSession | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Only own quizzes (not from community) and not already published
      const publishedQuizIds = new Set(published.map((p) => p.quiz_id));

      setMyQuizzes(
        sessions.filter(
          (s) => !s.source_published_quiz_id && !publishedQuizIds.has(s.id),
        ),
      );
  ...
  const handlePublish = async () => {
    ...
    setError("");
    setPublishing(true);
    const { error: pubError } = await publishQuiz(
      selectedQuiz.id,
      title.trim(),
      description.trim(),
    );
    ...
    const updated = await getMyPublishedQuizzes();
    setPublishedQuizzes(updated);
    setMyQuizzes((prev) => prev.filter((q) => q.id !== selectedQuiz.id));
    setTitle("");
    setDescription("");
  ...
  const handleUnpublish = async (publishedQuizId: string) => {
    await unpublishQuiz(publishedQuizId);
    const removed = publishedQuizzes.find((p) => p.id === publishedQuizId);
    setPublishedQuizzes((prev) => prev.filter((p) => p.id !== publishedQuizId));
    if (removed) {
      const sessions = await getQuizSessions();
      const match = sessions.find((s) => s.id === removed.quiz_id);
      if (match) setMyQuizzes((prev) => [match, ...prev]);
    }
  };
    ...
    <>
      ...
      <div className="relative z-10 min-h-screen bg-background text-foreground">
        <div className="max-w-3xl mx-auto px-6 pt-20 pb-24">
          ...
          {/* ── Publish form ── */}
          <div
            className="page-enter stagger-1 mb-10"
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  <label
                    ...
                    style={{ color: `rgb(var(--theme-glow) / 0.45)` }}
                  >
                    // public title <span style={{ color: "#ef4444" }}>*</span>
                  </label>
                  <input
                    className="themed-input"
                    placeholder="How others will see this quiz..."
                    value={title}
                    onChange={(e) => {
                ...
                <div>
                  ...
                  >
                    ...
                    <textarea
                      ...
                      placeholder="What topic does this quiz cover? Who is it for..."
                      value={description}
                      maxLength={300}
                      onChange={(e) => {
                      ...
                      onFocus={(e) => {
                        const w = e.currentTarget.closest(
                          ".rounded-xl",
                        ) as HTMLElement;
                      ...
                      onBlur={(e) => {
                        ...
                        if (w)
                          w.style.borderColor = `rgb(var(--theme-glow) / 0.2)`;
                      }}
                    />
                  </div>
                </div>
                    ...
                    >
                      <div className="flex items-center gap-2 font-mono text-xs">
                        <Menu
                          ...
                          style={{ color: "var(--theme-primary)" }}
                        />
                        {selectedQuiz ? selectedQuiz.title : "choose_quiz..."}
                      </div>
                      <ChevronDown
                    ...
                    {dropdownOpen && (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="w-1.5 h-1.5 rounded-full bg-yellow-400/40" />
                          <span className="w-1.5 h-1.5 rounded-full bg-green-400/40" />
                          <span className="ml-2">my_quizzes.sh</span>
                        </div>
                ...
                <button
                  ...
                  disabled={publishing}
                  className="publish-btn w-full py-3.5 rounded-[3px] font-semibold text-sm flex items-center justify-center gap-2"
                >
                  {publishing ? (
                    <>
                      <span
                    ...
                    <div className="flex items-center font-mono font-bold gap-3">
                      <Plus className="w-4 h-4" /> publish_quiz
                    </div>
                  )}
                </button>
              </div>
          ...
          <div className="page-enter stagger-2">
            ...
            {loading ? (
              ...
              <div className="text-center py-12 font-mono">
                ...
                >
                  <span style={{ color: "var(--theme-primary)" }}>$</span> ls
                  -la published/
                  <br />
