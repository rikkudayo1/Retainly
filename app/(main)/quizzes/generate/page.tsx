...
import { createQuiz, saveQuizScore } from "@/lib/db";
...
const QuizGenerationPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (quizErr || !quiz) {
        setError(quizErr ?? "Failed to save quiz.");
        setSaving(false);
        setLoading(false);
        return;
      }
        ...
        <div className="w-full max-w-xl space-y-8 page-enter">
          ...
          <div className="rounded-2xl border overflow-hidden"
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl border overflow-hidden" style={{ borderColor: `rgb(var(--theme-glow) / 0.15)` }}>
                  <div className="flex items-center gap-2 px-3 py-2 border-b font-mono text-[10px]"
                    style={{ borderColor: `rgb(var(--theme-glow) / 0.08)`, backgroundColor: `rgb(var(--theme-glow) / 0.03)`, color: `rgb(var(--theme-glow) / 0.35)` }}>
                    <span style={{ color: "var(--theme-primary)" }}>$</span>
                    <span>description <span style={{ color: `rgb(var(--theme-glow) / 0.3)` }}>(optional)</span></span>
                  </div>
                  <input
                    className="w-full px-4 py-2.5 text-sm outline-none bg-transparent placeholder:text-muted-foreground/30"
                    style={{ color: "var(--foreground)" }}
              ...
              {error && (
                <p className="font-mono text-xs" style={{ color: "rgb(239 68 68)" }}>// error: {error}</p>
              )}
              ...
              <button onClick={handleGenerate}
                ...
                {loading || saving
                  ? <span className="font-mono text-xs">{saving ? "saving..." : "generating..."}</span>
                  : <><Plus className="w-4 h-4" /> <span className="font-mono">{t("quiz.generate")}</span></>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="min-h-screen bg-background text-foreground flex items-center justify-center px-6">
          <div className="w-full max-w-xl space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4 font-mono text-xs" style={{ color: `rgb(var(--theme-glow) / 0.4)` }}>
            ...
            <div className="relative h-0.5 rounded-full overflow-hidden" style={{ backgroundColor: `rgb(var(--theme-glow) / 0.1)` }}>
            ...
            {answered && (
              <div className="explanation-enter space-y-3">
                ...
                <button onClick={handleNext}
                  className="w-full py-3 rounded-[3px] font-bold text-sm transition-all flex items-center justify-center gap-2 hover:opacity-90"
                  style={{ background: "var(--theme-primary)", color: "#fff" }}>
                  {currentIndex + 1 >= questions.length
                    ? <><Trophy className="w-4 h-4" /> {t("quiz.results")}</>
                    : <span className="font-mono flex items-center gap-3">{t("quiz.next")} <ArrowRight className="w-4 h-4" /></span>}
                </button>
              </div>
            )}
          </div>
            ...
            <div className="flex gap-3">
              ...
              {savedId && (
                <button onClick={() => router.push("/quizzes")}
                  ...
                  style={{ backgroundColor: "rgb(34 197 94 / 0.1)", color: "#22c55e", border: "1px solid rgb(34 197 94 / 0.3)" }}>
                  ✓ saved · view all
                </button>
              )}
            </div>
