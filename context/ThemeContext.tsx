...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [unlockedThemes, setUnlockedThemes] = useState<ColorTheme[]>(["blue", "purple", "luxury"]);

  useEffect(() => {
    const savedColor = localStorage.getItem("retainly_color_theme") as ColorTheme;
    const savedMode = localStorage.getItem("retainly_mode") as Mode;
    const savedUnlocked = localStorage.getItem("retainly_unlocked_themes");
  ...
  const setColorTheme = (t: ColorTheme) => {
    ...
    localStorage.setItem("retainly_color_theme", t);
  };

  const toggleMode = () => {
    const next = mode === "dark" ? "light" : "dark";
    setMode(next);
    localStorage.setItem("retainly_mode", next);
  };
  ...
  const unlockTheme = (t: ColorTheme) => {
    ...
    localStorage.setItem("retainly_unlocked_themes", JSON.stringify(updated));
  };

  return (
    <ThemeContext.Provider value={{ colorTheme, mode, unlockedThemes, setColorTheme, toggleMode, unlockTheme }}>
      {children}
