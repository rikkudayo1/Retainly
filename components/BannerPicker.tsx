...
const BannerPicker = ({ currentBannerId, onChanged }: BannerPickerProps) => {
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      >
        <DrawerHeader>
          <DrawerTitle className="text-foreground font-black">
            Choose Banner
          </DrawerTitle>
        </DrawerHeader>
            ...
            {BANNERS.map((banner) => {
              ...
              return (
                <button
                  ...
                  style={{ height: 80 }}
                >
                  {/* Gradient preview — banner-preview class keeps gradient intact in pixel mode */}
                  <div
                    className="w-full h-full banner-preview"
                    style={{ background: banner.gradient }}
                  />

                  {/* Label — banner-overlay exempts it from any blanket bg overrides */}
                  <div
                    className="banner-overlay absolute bottom-0 left-0 right-0 px-2 py-1 text-[10px] font-semibold text-white"
                    style={{
                      backgroundColor: "rgba(0,0,0,0.4)",
                  ...
                  {/* Selected state — scrim + checkmark */}
                  {isSelected && (
                    <>
                      <div
                        className="banner-overlay absolute inset-0 flex items-center justify-center"
                      ...
                      {/* Selected ring */}
                      <div
                        className="banner-overlay absolute inset-0 rounded-2xl border-2 pointer-events-none"
